...
- Expanded CP3 message-tool parity depth with explicit `message` action handling (`send`, `poll`, `react`, `reactions`, `read`, `edit`, `delete`) plus bounded in-memory reaction/edit/delete state, corpus fixtures, and dedicated runtime regression coverage.
