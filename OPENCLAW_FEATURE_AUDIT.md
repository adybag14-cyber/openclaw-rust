...
- Added cron webhook delivery parity hardening: cron callbacks now mirror upstream `finished` event payload shape, require non-empty summaries, attach optional `Authorization: Bearer <cron.webhookToken>` headers, and support legacy `notify: true` fallback via global `cron.webhook` config.
