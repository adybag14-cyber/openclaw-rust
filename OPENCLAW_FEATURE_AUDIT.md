...
- Added OpenCode + ZhipuAI provider parity updates: default model catalog now includes OpenCode Zen `glm-5-free` and ZhipuAI `glm-5`, provider normalization adds `zhipu`/`bigmodel` aliases (`zhipuai`, `zhipuai-coding`), provider runtime defaults include official Zhipu OpenAI-compatible endpoints (`/api/paas/v4`, `/api/coding/paas/v4`), and OAuth provider catalog exposure includes OpenCode/Zhipu entries for setup discovery.
