...
- Added CP8 starter hardening gate runners (`scripts/parity/run-cp8-gate.ps1`, `scripts/parity/run-cp8-gate.sh`) for replay/soak reliability + security-regression fixtures.
- Added CP8 CI gate job (`hardening-cp8`) with published artifacts (`parity/generated/cp8/*`) for trend tracking of reliability/security fixture runtime.
