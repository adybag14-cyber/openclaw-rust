...
- Added dedicated CP3 gate runners (`scripts/parity/run-cp3-gate.ps1`, `scripts/parity/run-cp3-gate.sh`) plus CI job (`tool-runtime-cp3`) with summary/artifact upload.
...
- `Tool execution layer`: Upstream OpenClaw feature surface is `exec`, `process`, `apply_patch`, browser/canvas/nodes, message, gateway, and sessions\_\* methods. Rust status is `Partial`. Notes: Rust now includes CP3 policy-precedence + loop-guard foundations and enforces deny decisions in defender classification; full Rust-native tool host/runtime execution parity is still pending.
