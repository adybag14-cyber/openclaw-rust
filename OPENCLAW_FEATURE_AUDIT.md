...
- `Channel driver abstraction`: Upstream OpenClaw feature surface is channel-specific frame parsing adapters. Rust status is `Partial`. Notes: Trait-based registry added with `whatsapp`, `telegram`, `slack`, `discord`, `irc`, `signal`, `imessage`, `webchat`, `bluebubbles`, `googlechat`, `msteams`, `matrix`, `zalo`, `zalouser`, and generic drivers.
...
2. Complete CP4 transport lifecycle + webhook ingress parity for Wave-1 and Wave-2 channels on top of the channel-runtime helper layer.
