...
- `Full Gateway replacement`: Upstream OpenClaw feature surface is sessions, presence, routing, config mutations, cron/webhooks, and control UI serving. Rust status is `Partial`. Notes: Rust now covers a broad in-memory gateway RPC surface (including cron CRUD/run/runs/status, standalone auto due-run scheduling, webhook delivery mode callbacks, and legacy notify fallback semantics), but TS still owns durable cron persistence across restarts and UI serving/runtime orchestration.
...
- `Tool execution layer`: Upstream OpenClaw feature surface is `exec`, `process`, `apply_patch`, browser/canvas/nodes, message, gateway, and sessions\_\* methods. Rust status is `Partial`. Notes: CP3 host parity now covers `exec/process/read/write/edit/apply_patch` plus Rust-native `gateway`/`sessions`/`message` families (gateway introspection + bounded session message history/list/reset workflows + message-send alias behavior) with corpus + gate fixtures; broader browser/canvas/nodes tool-family execution parity remains pending.
...
- `Voice stack`: Upstream OpenClaw feature surface is Voice Wake, Talk Mode, and audio I/O flows. Rust status is `Partial`. Notes: Talk mode, `tts.*`, and VoiceWake control-plane methods (`voicewake.get`, `voicewake.set`) are available in-memory, and `tts.convert` now emits deterministic synthesized audio payload metadata (`audioBase64`, byte count, duration, sample rate, channel count) for runtime consumers; full live audio capture/playback device flows remain out of current Rust scope.
...
- `CLI + control surface`: Upstream OpenClaw feature surface is operator CLI command parity, `doctor` diagnostics, and control UI compatibility pathways. Rust status is `Implemented`. Notes: Rust now exposes gateway/agent/message/nodes/sessions command families plus `doctor`, with CP7 gate fixtures and CI artifact enforcement; full UI serving remains tracked under full gateway replacement.
...
- `Channel driver abstraction`: Upstream OpenClaw feature surface is channel-specific frame parsing adapters. Rust status is `Partial`. Notes: Trait-based registry added with `whatsapp`, `telegram`, `slack`, `discord`, `irc`, `signal`, `imessage`, `webchat`, `bluebubbles`, `googlechat`, `msteams`, `matrix`, `zalo`, `zalouser`, `feishu`, `mattermost`, `line`, `nextcloud-talk`, `nostr`, `tlon`, and generic drivers.
